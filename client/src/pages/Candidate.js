...
const handleCloseModal = () => {
  ...
  const assessmentDataTechnical = [
    { no: '1', assessment: 'Quantitative Test', marks: '10', duration: '20min' },
    { no: '2', assessment: 'Vocabulary Test', marks: '10', duration: '10min' },
    { no: '3', assessment: 'Psychometric Test', marks: '10', duration: '15min' },
  ...
  const assessmentData = auth.selectedCategory === 'Technical' ? assessmentDataTechnical : assessmentDataNonTechnical;
    ...
    {showModal && (
      <div className="modal">
        <div className="modal-content">
          ...
          <ol>
              <li>Please check your Name & Personal Details on the Left side of the portal</li>
              <li>Applicant will get chance to attempt One test one time only</li>
              <li>If Personal Details are not match with yours, please Logout immediately and contact HR</li>
              ...
              <hr style={{width: '100%'}}/>
              <center><h3 style={{color: 'red', fontWeight: 'bold'}}>Keep in Mind:</h3></center>
              <li>In case during the test If you visit any other <span style={{color: 'red', fontWeight: 'bold'}}>BROWSER/TAB/WEBPAGE</span> that particular test will be - <span style={{color: 'red'}}>Disabled</span></li>
              <li>Use of Electronic Devices other than this Device on which you will be completing this exam– <span style={{color: 'red'}}>Not Permitted</span></li>
          </ol>
      ...
      <div style={{padding: '25px'}}>
          ...
          <div className='test-button-1'>
              ...
              <div>
                <button className='button' onClick={handleClick3} disabled={auth.vocabulary >= 0} type="submit">Vocabulary</button>
              </div>
              <div>
                <button className='button' onClick={handleClick2} disabled={auth.quantitative >= 0} type="submit">Quantitative</button>
              </div>
          ...
          {auth.selectedCategory === 'Technical' && (
            <div>
              <div>
                <button className='button' onClick={handleClick1} disabled={auth.psychometric >= 0} type="button">Psychometric</button>
              </div>
              <div>
                <button className='button' onClick={handleClick4} disabled={auth.java >= 0} type="button">Java</button>
              </div>
            </div>
          )}
          {auth.selectedCategory === 'Non-Technical' && (
            <div>
              ...
              <div>
              <button className='button' onClick={handleClick5} disabled={auth.accounts >= 0} type="button">Accounts</button>
            </div>
            <div>
              <button className='button' onClick={handleClick6} disabled={auth.excel >= 0} type="button">Excel</button>
            </div>
        ...
        <div className='testcard'>
          ...
          <p className='description' style={{ color: "black" }}> I have successfully submitted all tests </p>
        </div>
        <div className='but-cont'>
          <button className='sub' style={{ margin: '0 auto' }} type="submit" disabled={!isChecked} onClick={handleSubmit} >Submit</button>
        </div>
        </div>
      </div>
  )
