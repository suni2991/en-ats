...
const Applicant = () => {
  ...
  const fetchCandidates = async () => {
    try {
      const response = await axios.get(`${URL}/api/candidatesreport`, {
        headers: {
  ...
  const fetchOnboardedCandidates = async () => {
    try {
      const response = await axios.get(`${URL}/api/candidate/Onboarded`);
  ...
  const deleteCandidate = async (id) => {
    try {
      await axios.delete(`${URL}/api/candidate/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
