...
const JobDashboard = () => {
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const departmentMatch = job.department.toLowerCase().includes(searchQuery.toLowerCase());
    const locationMatch = job.jobLocation.toLowerCase().includes(searchQuery.toLowerCase());
  
    // Fix for primarySkills being an array
    const skillsMatch = Array.isArray(job.primarySkills) 
      ? job.primarySkills.some(skill => skill.toLowerCase().includes(searchQuery.toLowerCase()))
      : typeof job.primarySkills === 'string' && job.primarySkills.toLowerCase().includes(searchQuery.toLowerCase());
  
    return positionMatch || departmentMatch || locationMatch || skillsMatch;
  });
