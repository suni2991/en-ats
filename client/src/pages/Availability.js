...
const Availability = () => {
  ...
  const handleDelete = async (slot) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await fetch(`${URL}/api/candidate/slot/${slot._id}`, {
            method: 'DELETE',
            headers: {
  ...
  const fetchSlots = async () => {
    try {
      const response = await fetch(`${URL}/api/candidate/profile/${candidateId}`);
  ...
  const handleOk = async () => {
    try {
      ...
      const response = await fetch(`${URL}/api/candidate/slot/${selectedSlot?._id || ''}`, {
