...
const Schedule = () => {
  ...
  const handleBookSlotSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${URL}/api/candidates/${selectedCandidate._id}/availability`, payload, {
        headers: {
  ...
  const handleRequest = async () => {
    ...
    try {
      const response = await fetch(`${URL}/api/slot/request`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="vh-page">
      <Fetchtable 
        url={`${URL}/api/panelists/enfusian`}
        columns={userColumns}
      />
