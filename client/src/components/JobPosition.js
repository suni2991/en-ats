...
const JobPositionPieChart = ({ department }) => {
      ...
      try {
        if (selectedDepartment) {
          ...
          const response = await axios.get(
            `${URL}/api/positions-with-vacancies/${selectedDepartment}`,
            {
              headers: {
          ...
          await Promise.all(
            response.data.positions.map(async (pos) => {
              const res = await axios.get(
                `${URL}/api/vacancy-status/${pos.position}`,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
  ...
  //       if (clickedPosition) {
  //         const response = await axios.get(
  //           `${URL}/api/vacancy-status/${clickedPosition[0]?.position}`,
  //           {
  //             headers: {
  //               Authorization: `Bearer ${token}`,
  //             },
  ...
  const fetchPositionData = async (positionToFetch) => {
    try {
      if (clickedPosition) {
        const response = await axios.get(
          `${URL}/api/vacancy-status/${positionToFetch}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      const vacancyStatusPromises = positionsData.map(async (position) => {
        const response = await axios.get(
          `${URL}/api/vacancy-status/${position.position}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
