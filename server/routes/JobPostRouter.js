...
jobRouter.get("/api/job/position/:name", async (req, res) => {
...
jobRouter.get(
  "/api/positions-with-vacancies/:department",
  authenticate,
  checkPermission("view_positions_with_vacancies_by_department"),
  async (req, res) => {
    const department = req.params.department;
    try {
      const positions = await Job.find(
        { department: department, vacancies: { $gt: 0 } },
        "position vacancies"
      );
      res.json({ positions });
    } catch (err) {
...
jobRouter.put("/api/job-posts/:id",
  // authenticate,
