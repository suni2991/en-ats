...
userRouter.get(
  ...
  async (req, res) => {
    const docs = await Candidate.find({
      role: { $in: ["HR", "Panelist", "Hiring-Manager"] },
    });
    res.json(docs);
  }
);
  ...
  async (req, res) => {
    try {
      const panelists = await Candidate.find(
        { role: { $in: ["Panelist", "Hiring-Manager", "HR"] } },
...
userRouter.put('/candidate/:id/reset-scores', async (req, res) => {
  try {
    const _id = req.params.id;
...
userRouter.delete('/candidate/slot/:slotId', async (req, res) => {
